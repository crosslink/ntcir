...
/*
	...
	--------------
*/
inline static char *string_clean_tag(char *file, long trim = 0)
{
char *ch, *from, *to;
...
inline static char *string_strip_tags(char *file, long lower_case_only = 1)
{
char *ch, *from, *to;
char *next2, *next;
char tmp[10];
...
while (*ch != '\0')
	...
	if (*ch == '<')			// then remove the XML tags
		...
		*ch++ = ' ';
		...
		next = ch + 1;
		if (*next != '\0' && isalpha(*next) && *(next2 = ch + 2) == '.') {
			(ch = next2)++;
		}
		else
			*ch++ = ' ';
...
//		*current++ = token;
	while(*source != '\0') {
		while(*source != '\0' && !(isalnum(*source) || *source == '-'))
			++source;

		if (*source != '\0') {
			*current++ = source;
			while(*source != '\0' && (isalnum(*source) || *source == '\'')) {
				if (*source == '\'' && *(source + 1) == 's')
					++source;
				++source;
			}
			if (*source != '\0') {
